...
.. code-block:: python
    ...
    m = Map([
        # Static URLs
        Rule('/', endpoint='staic/index'),
        Rule('/about', endpoint='staic/about'),
        Rule('/help', endpoint='staic/help'),
        # Knowledge Base
        Subdomain('kb', [
...
.. code-block:: python
    ...
    c.build("staic/index", force_external=True)
...
def parse_rule(rule: str) -> t.Iterator[t.Tuple[t.Optional[str], t.Optional[str], str]]:
    ...
    `None` it's a staic url part, otherwise it's a dynamic one.
...
class Rule(RuleFactory):
    ...
    `build_only`
        ...
        or folder that are not handled by the WSGI application (like staic data)
    ...
    def match_compare_key(
        ...
        2.  rules with more staic parts come first so the second argument
            is the negative length of the number of the staic weights.
        3.  we order by staic weights, which is a combination of index
            and length
